/*
Print the following Pattern: (Square + inside sqare)
	...
	public static void main(String[] args) {
		...
		for (int i = 1; i <= n; i++) { // Columns (i)
			...
			for (int j = 1; j <= n; j++) // Rows (j)
				if (i==1 || j==1 || i==n || j==n || (i==n/4 && (j>=n/4 && j<=n/2+n/3+1)) || (i==n/2+n/3+1 && (j>=n/4 && j<=n/2+n/3+1)) || (j==n/4 && (i>=n/4 && i<=n/2+n/3+1)) || (j==n/2+n/3+1 && (i>=n/4 && i<=n/2+n/3+1))) 
